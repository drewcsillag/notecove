...
describe('Sync Handlers', () => {
  ...
  describe('sync:getStatus', () => {
    it('should return sync status', async () => {
      ...
      const status = { synced: true, lastSync: Date.now() };
      mocks.database.getNoteSyncState.mockResolvedValue(status);
      const result = await (handlers as any).handleGetSyncStatus(mockEvent, noteId);

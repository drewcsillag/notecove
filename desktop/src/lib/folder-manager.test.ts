...
// Folder type (inferred from FolderManager methods)
interface Folder {
  id: string;
  name: string;
  parentId: string;
...
describe('FolderManager', () => {
  ...
  describe('initialization', () => {
    it('should initialize with default folders', () => {
      const folders: Folder[] = folderManager.getAllFolders();
    ...
    it('should have correct default folder structure', () => {
      const root: Folder | null = folderManager.getFolder('root');
      const allNotes: Folder | null = folderManager.getFolder('all-notes');
      const trash: Folder | null = folderManager.getFolder('trash');
  ...
  describe('folder creation', () => {
    it('should create a new folder', async () => {
      const folder: Folder = await folderManager.createFolder('Work', 'root');
    ...
    it('should create nested folders', async () => {
      const parent: Folder = await folderManager.createFolder('Projects', 'root');
      const child: Folder = await folderManager.createFolder('Personal', parent.id);
    ...
    it('should trim folder names', async () => {
      const folder: Folder = await folderManager.createFolder('  Trimmed  ', 'root');
    ...
    it('should notify listeners on folder creation', async () => {
      ...
      folderManager.addListener((event: string, data: any) => {
      ...
      const createEvent = events.find((e: any) => e.event === 'folder-created');
  ...
  describe('folder retrieval', () => {
    it('should get folder by ID', async () => {
      const created: Folder = await folderManager.createFolder('Test', 'root');
      const retrieved: Folder | null = folderManager.getFolder(created.id);

      expect(retrieved).toEqual(created);
    });

    it('should return null for non-existent folder', () => {
      const folder: Folder | null = folderManager.getFolder('non-existent-id');
    ...
    it('should get child folders', async () => {
      const parent: Folder = await folderManager.createFolder('Parent', 'root');
      await folderManager.createFolder('Child1', parent.id);
      await folderManager.createFolder('Child2', parent.id);

      const children: Folder[] = folderManager.getChildFolders(parent.id);
  ...
  describe('folder tree', () => {
    it('should build correct folder tree', async () => {
      ...
      const tree: any[] = folderManager.getFolderTree();

      expect(Array.isArray(tree)).toBe(true);
      const parentInTree = tree.find((f: any) => f.name === 'Parent');
    ...
    it('should build multi-level tree', async () => {
      const level1: Folder = await folderManager.createFolder('Level1', 'root');
      const level2: Folder = await folderManager.createFolder('Level2', level1.id);
      await folderManager.createFolder('Level3', level2.id);

      const tree: any[] = folderManager.getFolderTree();
      const l1 = tree.find((f: any) => f.name === 'Level1');
  ...
  describe('folder updates', () => {
    it('should update folder name', async () => {
      const folder: Folder = await folderManager.createFolder('OldName', 'root');
      const updated: Folder | null = await folderManager.updateFolder(folder.id, { name: 'NewName' });
    ...
    it('should update paths of child folders when parent renamed', async () => {
      const parent: Folder = await folderManager.createFolder('Parent', 'root');
      const child: Folder = await folderManager.createFolder('Child', parent.id);

      await folderManager.updateFolder(parent.id, { name: 'RenamedParent' });

      const updatedChild: Folder | null = folderManager.getFolder(child.id);
      expect(updatedChild!.path).toBe('RenamedParent/Child');
    });

    it('should not update special folders', async () => {
      const result: Folder | null = await folderManager.updateFolder('all-notes', { name: 'Changed' });
      expect(result).toBeNull();
    });

    it('should not update root folder', async () => {
      const result: Folder | null = await folderManager.updateFolder('root', { name: 'Changed' });
      expect(result).toBeNull();
    });

    it('should notify listeners on folder update', async () => {
      const folder: Folder = await folderManager.createFolder('Test', 'root');
      const events: Array<{ event: string; data: any }> = [];
      ...
      const updateEvent = events.find((e: any) => e.event === 'folder-updated');
  ...
  describe('folder deletion', () => {
    it('should delete a folder', async () => {
      const folder: Folder = await folderManager.createFolder('ToDelete', 'root');
      const success: boolean = await folderManager.deleteFolder(folder.id);
    ...
    it('should not delete special folders', async () => {
      const success: boolean = await folderManager.deleteFolder('all-notes');
      expect(success).toBe(false);
    });

    it('should not delete root folder', async () => {
      const success: boolean = await folderManager.deleteFolder('root');
      expect(success).toBe(false);
    });
    ...
    it('should notify listeners on folder deletion', async () => {
      ...
      const deleteEvent = events.find((e: any) => e.event === 'folder-deleted');
  ...
  describe('folder movement', () => {
    it('should move folder to new parent', async () => {
      const folder1: Folder = await folderManager.createFolder('Folder1', 'root');
      const folder2: Folder = await folderManager.createFolder('Folder2', 'root');
      const child: Folder = await folderManager.createFolder('Child', folder1.id);

      await folderManager.moveFolder(child.id, folder2.id);

      const moved: Folder | null = folderManager.getFolder(child.id);
    ...
    it('should not move folder into its own subtree', async () => {
      ...
      const result: Folder | null = await folderManager.moveFolder(parent.id, child.id);
      expect(result).toBeNull();
    });

    it('should not move special folders', async () => {
      const folder: Folder = await folderManager.createFolder('Test', 'root');
      const result: Folder | null = await folderManager.moveFolder('all-notes', folder.id);
      expect(result).toBeNull();
    });
    ...
    it('should update child paths when moved', async () => {
      const folder1: Folder = await folderManager.createFolder('Folder1', 'root');
      const folder2: Folder = await folderManager.createFolder('Folder2', 'root');
      const parent: Folder = await folderManager.createFolder('Parent', folder1.id);
      const child: Folder = await folderManager.createFolder('Child', parent.id);

      await folderManager.moveFolder(parent.id, folder2.id);

      const movedChild: Folder | null = folderManager.getFolder(child.id);
  ...
  describe('folder utilities', () => {
    it('should check if folder is descendant', async () => {
      const level1: Folder = await folderManager.createFolder('Level1', 'root');
      const level2: Folder = await folderManager.createFolder('Level2', level1.id);
      const level3: Folder = await folderManager.createFolder('Level3', level2.id);
    ...
    it('should generate breadcrumb path', async () => {
      ...
      const breadcrumb: Folder[] = folderManager.getBreadcrumb(level3.id);
    ...
    it('should return empty breadcrumb for root children', () => {
      const breadcrumb: Folder[] = folderManager.getBreadcrumb('all-notes');
  ...
  describe('folder moving', () => {
    it('should move a folder to a new parent', async () => {
      ...
      const moved: Folder | null = await folderManager.moveFolder(folder2.id, folder1.id);
    ...
    it('should update descendant paths when moving a folder', async () => {
      const parent: Folder = await folderManager.createFolder('Parent', 'root');
      const child: Folder = await folderManager.createFolder('Child', parent.id);
      const grandchild: Folder = await folderManager.createFolder('Grandchild', child.id);

      const newParent: Folder = await folderManager.createFolder('NewParent', 'root');
      await folderManager.moveFolder(child.id, newParent.id);

      const updatedChild: Folder | null = folderManager.getFolder(child.id);
      const updatedGrandchild: Folder | null = folderManager.getFolder(grandchild.id);
    ...
    it('should not allow moving a folder into itself', async () => {
      const folder: Folder = await folderManager.createFolder('Folder', 'root');

      const result: Folder | null = await folderManager.moveFolder(folder.id, folder.id);
    ...
    it('should not allow moving into trash or all-notes', async () => {
      ...
      const result1: Folder | null = await folderManager.moveFolder(folder.id, 'trash');
      const result2: Folder | null = await folderManager.moveFolder(folder.id, 'all-notes');
